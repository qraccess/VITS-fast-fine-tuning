...
def run(rank, n_gpus, hps):
  ...
  # load existing model
  if hps.train_type == "resuming":
      resume_model_dir = hps.resume_dir # directory of the models to be resumed
      max_ckpt = find_max_ckpt(resume_model_dir)
      G_resume_ckpt = os.path.join(resume_model_dir, "G_{}.pth".format(max_ckpt))
      D_resume_ckpt = os.path.join(resume_model_dir, "D_{}.pth".format(max_ckpt))
      _, _, _, iteration = utils.load_checkpoint(G_resume_ckpt, net_g, None, drop_speaker_emb=hps.drop_speaker_embed)
      _, _, _, _ = utils.load_checkpoint(D_resume_ckpt, net_d, None)
      epoch_str = iteration
      global_step = max_ckpt
  else:
